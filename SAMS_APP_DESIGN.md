...
| v1.10.0 | 2026-08-01 | **Audit Checklist Infrastructure (G5-G6-G7-G9) + Tier 2 Workflow + Tier 3 Admin (T3.1-T3.3).** 5 new models, 8 API routes (4 checklist + 4 template CRUD). 4 components: ChecklistTemplateSelector, AssessmentChecklistTab (+ evidence T2.2), AuditWorkflowBar (T2.3), AuditReportTab (T3.3). 7 tabs in assessment detail. T2.1: Finding→checklist dropdown. T3.1: Template CRUD admin UI with inline item management. T3.2: Auto-prompt checklist adoption on new assessment. T3.3: Printable audit report — TOR, checklist compliance table, control effectiveness, findings with linked items. |
