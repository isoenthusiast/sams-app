...
| v1.10.0 | 2026-08-01 | **Audit Checklist Infrastructure (G5-G6-G7-G9) + Tier 2 Workflow (T2.1-T2.3).** 5 new models: AuditEvidence, AuditChecklist2Requirement, AuditChecklistTemplate, AuditChecklistTemplateItem, AuditChecklistItem. Finding.checklistItemId FK. v_AuditTrace VIEW (3,384 rows). 4 seeded templates (124 items). 4 new API routes. 3 new components: ChecklistTemplateSelector, AssessmentChecklistTab, AuditWorkflowBar. 6th \"📋 Checklist\" tab. G7: requirement badges on control rows. **T2.1:** Finding form now has checklist item dropdown — findings can be linked to specific checklist items. **T2.2:** Evidence attachments on each checklist item row via polymorphic AttachmentList. **T2.3:** 4-step audit workflow progress bar (Adopt Checklist → Assign Controls → Execute Checklist → Record Findings) with ✓/○ completion indicators. |
