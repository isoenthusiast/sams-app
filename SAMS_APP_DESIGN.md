...
| v1.10.0 | 2026-08-01 | **Audit Checklist Infrastructure (G5-G6-G7-G9) + Tiers 2-3-5.** 5 new models, 12 API routes, 5 components, 7 assessment tabs. T2.1: Finding→checklist dropdown. T2.2: Evidence attachments per item. T2.3: 4-step workflow bar. T3.1: Template CRUD admin UI. T3.2: Auto-prompt adoption on new assessment. T3.3: Printable audit report. T5.1: Quick-add finding from checklist item row. T5.2: Inline auditor notes editor with voice input. T5.3: Auto-adopt templates on assessment creation. T5.4: NonCompliant status auto-prompts finding creation. |
