...
**Last Updated:** September 04, 2026 (v1.20.0 — SAMS-016: Master Content Roll-Forward — versioned content packs + provider adopts on the client's behalf; v1.19.0 — SAMS-015: Tamper-Evident Audit Trail — per-company hash chain + client-held weekly anchors; v1.18.0 — SAMS-012: SSO (Microsoft Entra ID, link-by-email) + force-password-change; v1.17.0 — SAMS-011: public read-only API — company-scoped bearer keys; v1.16.4 — SAMS-013: Transcript → Evidence Chain, human-confirmed extraction; v1.16.4 — SAMS-009: outbound notifications — per-company webhook transport + sweep + weekly digest; v1.16.3 — SAMS-010: white-label theming)
---
...
|-------|---------|------------|
| **User** | System user | name, username (unique), email, role (Admin/Superuser/Assessor/Interviewee), positionId (FK→Position), companyId, managerName, managerUsername, organisationIndicator. **v1.8.0:** Added managerName, organisationIdentifier fields. **v1.8.2:** Added managerUsername (resolved FK-like reference to User.username). **v1.13.15:** Added `providerRole` (`ProviderRole` enum, nullable, additive — **orthogonal** to the `role` enum; null = not provider staff). **v1.18.0 (SAMS-012):** Added `mustChangePassword` (`Boolean @default(false)` — wizard-provisioned users get it set; a credentials login with the flag redirects to `/change-password` until cleared; SSO users are NOT forced through it) |
...
| Model | Purpose | Key Fields |
|-------|---------|------------|
| **Company.notificationWebhookUrl** | Per-company incoming webhook URL (Slack-compatible `{"text": …}` — works on Slack + Teams legacy connectors). **WRITE-ONLY secret**: settable via the client-Admin portal settings card, MASKED in every API response (`{ configured }` only), NEVER logged to ActivityLog, NEVER included in a digest post, and excluded from the client-data export (exact name added to `EXCLUSION_COLUMNS`). Nullable — null = no webhook → the company receives no outbound posts. | `Company.notificationWebhookUrl String?` |
| **NotificationDelivery** | Fire-and-record audit row for EVERY outbound webhook attempt — proves "we posted it"; no retry queue in v1. Written for fabric events (once per event at the company channel), the sweep, the digest, and the portal "Send test". `postCompanyWebhook` NEVER throws (SAMS-006 fault-containment carries over). Cross-tenant safe by construction: only the EVENT's own company URL is read and targetted. | id (cuid), notificationId? (FK→Notification, `onDelete: SetNull`, null for company-channel/sweep/digest posts), channel (`NotificationDeliveryChannel` enum, default `webhook`), companyId (FK→Company, `onDelete: Cascade`), status (`NotificationDeliveryStatus` enum `sent\|failed`), responseCode Int? (null when the connection itself failed), attemptedAt, payloadPreview (≤200). Indexes: (companyId), (notificationId), (status, attemptedAt). |
| **NotificationType += ActionOverdue** | Sweep event type added to the existing in-app enum: an Action crossed its targetDate (newly overdue ≤24h). Posted to the company channel + an in-app notification to the company's client Admins. | `ActionOverdue` enum value (additive). |
...
| Model | Purpose |
|-------|---------|
| **ActivityLog** | Audit trail of all mutations. **v1.19.0 (SAMS-015):** added nullable `companyId` + `chainHash` — the per-company Tamper-Evident hash-chain discriminator (`chainHash = sha256(prevChainHash ‖ canonical row fields)`; canonical field set `id, timestamp, description, activityType, username, refTable, refRecord, beforeData, afterData, companyId`, JSON canonicalized with sorted-key stable stringify, ordering `(createdAt, id)`). The log remains a **GLOBAL** log by design; `companyId` is a per-row owning-company discriminator so the chain is verifiable per company (rows that resolve to no company → `companyId null` = chainless). See `docs/AUDIT_CHAIN.md`. |
...
| `/login` | Client | Public | **v1.18.0:** "Sign in with Microsoft" (SSO, link-by-email) button above the credentials form. Denied SSO emails → `?error=sso_account_not_found`. |
| `/change-password` | Client | Auth | **v1.18.0:** Force-password-change (SAMS-012) — current + new ×2 (≥10 chars), verifies current (wrong → 400), clears `mustChangePassword`, refreshes the session JWT and continues. The middleware proxy reroutes a flagged credentials user here until the flag clears (direct-URL bypass → redirected back). |
...
| `/operator/onboarding` | Client | Provider | **v1.16.0:** Pilot Onboarding Wizard (SAMS-008) — guided, dry-runnable provider motion to bring a new client company live: company → content → users → review & go-live. Provider-gated (`session.user.providerRole`); unauthenticated → `/login`, non-provider → 403 view. `OnboardingWizard.tsx` is the client stepper; each step dry-runs the write before committing. |
...
#### Pilot Onboarding APIs — v1.16.0 (SAMS-008, Phase 3a)

Provider-gated with `session.user.providerRole` (non-provider → 403). Every write step is `dryRun`-first: the wizard validates/previews and the client only commits when the report is clean. Temp passwords never appear in any API response except `/finalize`.

| Route | Method | Auth | Description |
|-------|--------|------|-------------|
| `/api/operator/onboarding/company` | POST | Provider | Step 1 — company basics. `{ companyID, companyName, dryRun }`. `dryRun=true` → validate-only: missing fields + duplicate `companyID` (409 code `DUPLICATE`). `dryRun=false` → create the company row; 409 if the ID was taken since the dry-run. |
| `/api/operator/onboarding/content` | GET | Provider | Step 2 inventory: the SMDS master pack + the read-only counts `previewBootstrap` says a commit would produce (the dry-run panel). |
| `/api/operator/onboarding/content` | POST | Provider | Step 2 — adopt content. `{ companyId, dryRun }`. `dryRun=true` → `previewBootstrap` counts (no write). `dryRun=false` → runs the EXISTING shared bootstrap (`@/lib/bootstrap.runBootstrap`, the single adoption path) for the company → per-table results. Never reimplements adoption. |
| `/api/operator/onboarding/users` | POST | Provider | Step 3 — provision users. `{ companyId, rows, dryRun }`. `dryRun=true` → `validateUserRows` per-row report (N valid, duplicates existing/batch, invalid roles, missing name/username, unresolved managers + manager-resolution rate); `blocked=true` when duplicates/invalid-roles/missing-fields exist (unresolved managers are a warning, not a block). `dryRun=false` → re-validates at the write boundary via the SAME `isProvisionBlocked` rule — refuse with 4xx (409 duplicates / 422 bad rows, ZERO writes) if the dry-run would have blocked, then `provisionUsers` TRANSACTIONALLY per company (a mid-batch failure rolls back to ZERO partial users); creates users (role default Assessor, `active=true`) + `UserCompany` mappings + bcrypt temp passwords held ONLY in a transient store. Response carries `{ created, wizardId, users, managerResolution }` — NEVER temp passwords. |
| `/api/operator/onboarding/finalize` | POST | Provider | Step 4 — review & go-live. `{ companyId, wizardId }`. Builds the consolidated `OnboardingReport` (content counts + user count + `approvedForGoLive`) and returns the temp passwords for the `wizardId` EXACTLY ONCE (`consumeTempPasswords` deletes the entry; a second call returns `[]`). This is the ONLY place a temp password is ever emitted. |
...
| Route | Method | Auth | Description |
|-------|--------|------|-------------|
| `/api/cron/notify-sweep` | POST | CRON_SECRET bearer | Sweeps Actions that crossed their `targetDate` in the last **24h** (still open, `closureDate null`). Per affected company: one summarized company-channel webhook card (if a webhook is configured) + an in-app `ActionOverdue` notification to the company's client Admins. **401 without a matching `Authorization: Bearer <CRON_SECRET>`** and **503 (fail-closed) if `CRON_SECRET` is unset**. Fire-and-record — a delivery failure never fails the sweep. Exempted from the NextAuth proxy so it is reachable server-to-server without a session. Trigger is external (Hermes box cron pilot v1; Railway cron documented as the production alternative). |
| `/api/cron/weekly-digest` | POST | CRON_SECRET bearer | For every company WITH a webhook configured, posts ONE weekly digest card: SOC coverage % (fully-comply / assessed, `#51`), new findings this week, overdue actions, open evidence requests. **401 without the secret** / 503 if unset. Fire-and-record. |
| `/api/portal/notifications-settings` | GET/POST | Client-Admin | Read/set/clear the company's WRITE-ONLY webhook URL. GET → `{ configured }` (never the URL). POST `{ webhookUrl }` → set (validated http(s), ≤1000 chars); POST `{ clear: true }` → clear. Response is MASKED (`{ configured }` only) — the URL is never returned. Gate: `role=Admin` + company membership; the company resolves like a portal page (param > cookie > home > first). |
| `/api/portal/notifications-settings/test` | POST | Client-Admin | "Send test" — posts a test card to the company's webhook and reports `{ ok, status?, message }` (never the URL). Gate: client-Admin + company membership. |

(Single note: the webhook URL is excluded from the client-data export via `EXCLUSION_COLUMNS`; it never appears in ActivityLog, a digest post, or any API response.)
...
| `/api/auth/change-password` | POST | **v1.18.0:** Force-password-change (SAMS-012) — `requireAuth`; verifies current password (wrong → 400), new ≥10 chars + confirm (else 400), sets hash, clears `mustChangePassword`. Static route that takes precedence over the `[...nextauth]` catch-all. |

#### Public read-only API (`/api/public/v1/*`) — SAMS-011 (Phase 3b Feature B)

**Scope-by-construction:** the bearer key's `companyId` IS the query scope — no `?companyId=` param, no override, ever; a key can never cross tenants. Auth: `Authorization: Bearer <sams_pub_…>` (bcrypt hash-compare of active unrevoked keys; no rate limit v1). Uniform error `{ error }`: 401 missing/invalid key, 403 revoked key.

| Route | Method | Auth | Description |
|-------|--------|------|-------------|
| `/api/public/v1/soc` | GET | Bearer key | Company SOC — #51 coverage per process-area + overall (fully/partially/notComply, notAssessed, total, coveragePct). |
| `/api/public/v1/findings` | GET | Bearer key | Company findings; `?status=open` (≥1 unresolved action) / `closed` (otherwise) / all (absent); undefined status → empty list (documented). |
| `/api/public/v1/actions` | GET | Bearer key | Company actions; `?overdue=true` (open AND past targetDate); absent/false = all. |
| `/api/admin/api-keys` | POST/GET | Client Admin (of company) / Provider | Create (returns plaintext ONCE; only bcrypt hash stored) / List (label, createdAt, lastUsedAt, revokedAt, creator — NEVER key material). |
| `/api/admin/api-keys/[id]` | DELETE | Client Admin (of company) / Provider | Revoke (sets `revokedAt`; idempotent). |
...
- **v1.18.0 (SAMS-012):** the JWT carries `mustChangePassword` (credentials users) and the SSO `sso` marker; the middleware proxy force-gates a flagged credentials user onto `/change-password` until the flag is cleared. SSO sessions carry the real DB user `id`/`role` (link-by-email graft) — never the IdP subject id.
...
|---|--------|-------|-------------|
| 1 | **Login** | `/login` | **v1.18.0:** "Sign in with Microsoft" (SSO) button + Username/password form, SAMS branding |
...
|---|---|---|
| v1.20.0 | 2026-09-04 | **Master Content Roll-Forward — versioned content packs + provider adopts on the client's behalf (SAMS-016, Phase 4 Feature D).** Master content improvements reach live tenants deliberately (principle #29 — never continuous sync), with the operator reviewing a diff and adopting. **(1) Additive schema** (`scripts/db/migrations/20260904_add_content_pack.ts`, idempotent, no `prisma db push`, run ×2): `ContentStatus` enum (`Active | Superseded`), `contentStatus` + `supersededAt` on `Standard`/`ProcessArea`/`Requirement`/`Control`, `ContentPack` table (versioned, immutable, master-plane snapshot JSONB; unique `[companyId, version]`), `CompanyContentState` table (tenant-scoped, unique `companyId`; `contentVersion`, `lastPackId`, `lastAdoptedAt`, `acknowledgedContentVersion`), `NotificationType += ContentBaselineUpdated`, `ActivityLogType` rows `CONTENT_PACK_PUBLISH` / `CONTENT_PACK_ADOPT`. **(2) Diff/adopt engine** (`src/lib/content-rollforward.ts`): snapshot keyed by stable cross-tenant keys (Standard.standard / PA name / Requirement.requirementId / Control.controlRef); diff classifies added / changed / conflict (changed-elsewhere) / removed against the tenant's last-adopted pack; adopt is a selective apply by stable key — never the destructive `runBootstrap` wipe path (Conan binding #1) and never touches client data (full-row checksum proof). Removed-but-referenced content is retained Superseded (read-only, FK links resolve; never hard-deleted). **(3) Surfaces:** operator console per-client `Content vN · update vM` + Review diff → Adopt; client portal banner "Content baseline updated vN→vM" until acknowledged (persisted per company) + in-app `ContentBaselineUpdated` notification + company webhook notice. **(4) Export:** the client-data export manifest now carries `contentVersion`. **(5) Docs:** `docs/content-rollforward.md`. |
| v1.19.0 | 2026-09-04 | **Tamper-Evident Audit Trail — per-company hash chain + client-held weekly anchors (SAMS-015, Phase 4 Feature C).** The ActivityLog is append-only by convention; SAMS-015 makes it tamper-evident — and with the weekly anchor held by the client, tamper-proof against operator-side rewriting. **(1) Additive schema** (`scripts/db/migrations/20260904_add_audit_chain.ts`, idempotent, **no `prisma db push`**, run ×2, plus inline backfill): `ActivityLog.companyId String?` (indexed — per-row owning-company discriminator; the log remains a **GLOBAL** log by design, `companyId` is the discriminator so the chain can be verified per company; rows that resolve to no company → `companyId null` = chainless) + `ActivityLog.chainHash String?` (indexed). **Backfill** resolves `companyId` from `refTable`→`refRecord` (Assessment/User/Finding/Action/EvidenceRequest/ApiKey/Control/Company-return-null for global/unknown rows) in keyset batches (resumable — re-run skips already-assigned rows), then computes the per-company chain in `(createdAt, id)` order, and emits **per-refTable resolved/unresolved resolution stats** (Conan condition #1: a high unresolved share is surfaced in the review handoff). **(2) Shared chain engine** (`src/lib/audit-chain.ts` — the single source of truth used identically by writer, backfill, verifier and anchor): pinned canonical form (field set `id, timestamp, description, activityType, username, refTable, refRecord, beforeData, afterData, companyId`; JSON canonicalized with sorted-key stable stringify; separator U+2016; `chainHash = sha256(prevChainHash ‖ canonical)`), pinned ordering `(createdAt, id)`, and a per-company concurrency mechanism (`pg_advisory_xact_lock(hashtext(companyId))` serializes the write — which also covers the empty-chain first write — plus the head row is read `FOR UPDATE`); `chainHash` computed in the **same transaction** as the write. **(3) Every ActivityLog write path is now CHAINED** — `logActivity` in `src/lib/activity-log.ts` and the authz raw-INSERT path both route through `createChainedActivityLog`; the `admin/reset-health` direct-create now routes through it too (global Control reset → chainless by design). **(4) Verify CLI** `scripts/verify-audit-chain.ts <companyId>` — recomputes the company's chain, `exit 0` OK / `exit 1` with the FIRST broken row id; no-arg / unknown companyId → clear usage/error + `exit 2` (auditor-facing); reads only `WHERE companyId=<id>` (A's verify never touches B's rows). **(5) Weekly anchor rides the SAMS-009 digest** — `runWeeklyDigest` appends `auditAnchor: <company chain-head hash>` to each webhook post (`postCompanyWebhook` gained an `extra` payload field) so the CLIENT holds an independent copy; per-company (A's post carries only A's head hash); a delivery audit row is recorded as before. **(6) Isolation:** `ActivityLog` added to `model_matrix.json` (direct/companyId scope) — required because the suite flags any model with a `companyId` column missing from the matrix; the log stays **global by design** (rationale noted). **(7) Tests:** `scripts/audit-chain/seed.ts` + `functional_test.mts` + `webhook_receiver.mjs` + `start_test_server.sh` (owner plan a–f: backfill→verify exit 0 for every company, new API write extends the chain (hash continuity), tamper proof — edit one row → verify fails at exactly that row / delete a row → verify fails at the gap, digest webhook payload carries `auditAnchor` per-company, per-company scope (A's verify output contains zero B rows), all write paths chained). Test gate: `tsc` exit 0, `db:parity` empty migration, `build` exit 0, `test:isolation` 124/0, migration idempotent ×2 (exit 0/0), functional 33/0. Generated Prisma client committed. See `docs/AUDIT_CHAIN.md`. |
| v1.18.1 | 2026-09-04 | **MIC Ritual — quarterly SOC attestations with server-computed snapshots (SAMS-014, Phase 4 Feature B).** Management-in-control becomes a recurring, recorded ceremony. **(1) Additive schema** (`scripts/db/migrations/20260904_add_mic_attestation.ts`, idempotent, **no `prisma db push`**; verified ×2): `Company.attestationCadenceDays Int?` (per-company cadence knob; null = quarterly default 90), `MicAttestation` table (`companyId` FK→Company Cascade, `processAreaId` FK→ProcessArea Cascade, `attestedByUserId` FK→User SetNull, `attestedAt`, `socSnapshot` Json, `period String?`; indexes companyId/processAreaId/attestedAt), `NotificationType += MicAttestationDue`, and `ActivityLogType` rows `MIC_ATTEST` + `MIC_CADENCE_CHANGE`. **(2) Derived states are pure** (`src/lib/mic-attestations.ts`): `nextDue = (lastAttestedAt ?? Company.createdAt) + cadenceDays`; `state = nextDue < now ? overdue : nextDue <= now + 14d ? dueSoon : attested` — **no stored flags, no scheduler** (settled #3). **(3) SOFT enforcement (settled #1):** overdue surfaces on the `/fla` SOC dashboard per-PA chip, the `/portal` per-PA status, and the weekly-digest line `Overdue SOC attestations: N`; **NOTHING blocks**. **(4) Server-computed snapshot (settled #2, negative path):** `computePaSocSnapshot` returns `{ coveragePct (% FullyComply of assessed), findingCount (open findings), overdueActionCount (open + past target) }` computed SERVER-SIDE at signing — the attest route NEVER reads a client-supplied `socSnapshot` (tampered → ignored/overwritten). **(5) Routes:** `GET+POST /api/admin/processareas/[id]/attest` (GET = preview snapshot+state; POST = sign + `MIC_ATTEST` audit row; gate = client Admin/Superuser/Assessor who is a MEMBER of the PA's company via the stricter `hasMicCompanyAccess` — cross-tenant → 403 for EVERY role incl. Admin) and `PATCH /api/admin/companies/[id]/attestation-cadence` (client Admin/Superuser member OR provider; derived next-due recomputes immediately; `MIC_CADENCE_CHANGE` audit). **(6) SPO notify (settled #4):** `runAttestationDueNotifications()` in `notifications-outbound.ts`, called from the existing `/api/cron/weekly-digest` route (the only recurring sweep, NOT webhook-gated), emits an in-app `MicAttestationDue` to each company's SPOs for every PA whose derived state == overdue, deduped per cadence window (exactly-once — no re-spam while a PA stays overdue; re-arms once attested). **(7) UI:** `/setup/processdetails/[id]` gains a SOC-Attestation card (overdue/due-soon/attested chip + Attest button) and a modal that fetches the server snapshot then signs, flipping the chip; `/fla` per-PA chip; `/portal` per-PA status chip. **(8) Tests:** `scripts/mic-ritual/seed.ts` + `functional_test.mjs` + `verify_step.ts` + `ui_drive_test.mjs` (owner plan a–e + digest-line + exactly-once notify + driven f). Route + model matrices updated (`MicAttestation`, `admin/processareas/[id]/attest`, `admin/companies/[id]/attestation-cadence`) so `test:isolation` stays green by construction. Generated Prisma client committed. |
| v1.18.0 | 2026-09-04 | **SSO (Microsoft Entra ID, link-by-email) + force-password-change (Phase 3b Feature C, SAMS-012).** **(1) Additive schema:** `User.mustChangePassword Boolean @default(false)` (idempotent migration `scripts/db/migrations/20260904_add_user_must_change_password.ts`, ALTER TABLE ADD COLUMN IF NOT EXISTS, no `prisma db push`). **(2) SSO — Entra ID, GLOBAL one registration (settled #1):** NextAuth v5 `microsoft-entra-id` provider in `src/auth.ts`, env `AUTH_MICROSOFT_ENTRA_ID_ID/SECRET/TENANT` (Edward supplies at landing; `TENANT`→issuer `https://login.microsoftonline.com/<tenant>/v2.0`, `AUTH_MICROSOFT_ENTRA_ID_ISSUER` overrides). Registered ONLY when `AUTH_MICROSOFT_ENTRA_ID_ID` is present so the app boots/builds without the registration. **(3) Link-by-email, NO auto-provision (settled #2):** `src/lib/sso.ts` `resolveEntraSignIn` — known ACTIVE existing user matches the Entra email → allow; unknown / inactive / no-email → denied (`/login?error=sso_account_not_found`), NEVER creates an account. The `signIn` callback grafts the real DB user `id`/`role` onto the OAuth session (never the IdP subject id). **(4) Roles from `User.role` (settled #3)** — no IdP group mapping. **(5) Force-password-change (settled #4):** wizard-provisioned (SAMS-008) users get `mustChangePassword=true`; a credentials login with the flag is force-gated (middleware proxy `src/proxy.ts`) onto `/change-password` (current + new ×2 ≥10 chars, wrong current → 400) → clears flag → refreshes session JWT → continues; direct-URL bypass → redirected back. SSO users are NOT forced (they authenticate via their IdP; C4 describes credentials only). **(6) UI:** `/login` gains a "Sign in with Microsoft" button above the credentials form + SSO denial message; `src/app/change-password/page.tsx` + `/api/auth/change-password` route. Credentials login stays (admin recovery + non-SSO clients, settled #3). **(7) Tests:** `scripts/sso/unit_sso.test.mts` (mocked-profile + DB-backed link-by-email decisions, no-auto-provision — proves the signIn-callback logic since no Entra env), `scripts/sso/functional_test.mjs` (force-change flow: login → /change-password gate → wrong current 400 → correct change → flag cleared → portal accessible; direct-URL bypass; admin recovery; SSO button), `scripts/sso/ui_drive_test.mjs` (browser-drive of the force-change UI + session refresh), `scripts/sso/verify_step.ts` (macro: column exists default false, wizard provisioning sets flag, bcrypt-only hashes, SSO never auto-provisions). **Live SSO round-trip is LANDING-GATED on Edward's Entra app registration** (not yet in env); the signIn-callback logic is proven at unit level. Test gate: tsc 0, db:parity empty migration, build 0, test:isolation 124/0, functional 18/0, UI 11/0, unit 13/0, verify 6/0. Prisma client regenerated + committed. |
| v1.17.0 | 2026-09-04 | **Public read-only API — company-scoped bearer keys (SAMS-011, Phase 3b Feature B).** A minimal, key-scoped, READ-ONLY REST surface so pilot clients can pull their posture into their own dashboard / Power BI. **(1) Additive schema:** `ApiKey` table (`id`, `companyId` FK→Company ON DELETE CASCADE, `keyHash` (bcrypt — plaintext shown ONCE at creation, never stored), `label` ≤100, `createdAt`, `createdByUserId` FK→User ON DELETE SET NULL, `lastUsedAt?`, `revokedAt?`), index `(companyId)`. Additive idempotent migration `scripts/db/migrations/20260904_add_api_key.ts` (`CREATE TABLE IF NOT EXISTS` + index, no `prisma db push`, verified ×2). **Excluded from the client-data export** — `ApiKey` is deliberately absent from `EXPORT_TABLES` in `src/lib/data-trust-export.ts` (keys never travel with client data). **(2) Scope-by-construction (settled #2):** the key's `companyId` IS the query scope — no `?companyId=` param, no override, ever. A key can never cross tenants. **(3) Three read-only endpoints:** `GET /api/public/v1/soc` (#51 coverage per PA + overall), `GET /api/public/v1/findings?status=open|closed` (status is D-ERIVED from the action lifecycle — there is no `status` column on Finding: `open` = has ≥1 unresolved action, `closed` = otherwise; `?status=all`/absent = all; an undefined status returns an empty list, documented), `GET /api/public/v1/actions?overdue=true` (open AND past targetDate; absent/false = all). Writes stay app-only. **(4) Auth:** `Authorization: Bearer <sams_pub_…>` → bcrypt-hash-compare active unrevoked keys (the small key set is scanned because bcrypt is salted/non-indexable — acceptable at pilot scale; documented, no rate limit v1) → bump `lastUsedAt`. Uniform error shape `{error}`: 401 missing/invalid key, 403 revoked key. **(5) Key management (settled #5):** client Admin (of a company they belong to) + provider plane. `POST /api/admin/api-keys` (create — returns the PLAINTEXT ONCE, only the hash is stored), `GET /api/admin/api-keys?companyId=` (list — label/createdAt/lastUsedAt/revokedAt/creator only, NEVER key material), `DELETE /api/admin/api-keys/[id]` (revoke → sets `revokedAt`, idempotent). Cross-tenant create is refused by construction (Admin can only target a company they belong to; provider needs an explicit `companyId`). **(6) UI:** `src/app/portal/settings/page.tsx` gains a "Public read-only API" card (`PortalApiKeySettings.tsx`) — create (show-once box + copy), list (labels/dates/creator, Active/Revoked), revoke (confirm). The settings page is already client-Admin-gated (SAMS-010). **(7) Tests:** `scripts/public-api/seed.ts` (two companies w/ distinctive PUB001/PUB002 + ALPHA/BETA markers, SOC statuses, findings, an overdue + a non-overdue action, client Admins), `scripts/public-api/functional_test.mjs` (owner plan a–e over real HTTP: correct A-scoped JSON + lastUsedAt bump, cross-tenant scan → ZERO company-B identifiers, revoked→403 / wrong→401 / missing→401, list has no key material, export ZIP has no ApiKey), `scripts/public-api/verify_step.ts` (f: plaintext never in DB/logs — every keyHash is bcrypt, no `sams_pub_` substring in any ApiKey column or ActivityLog beforeData/afterData/description, plus a bcrypt round-trip proof; export manifest/exclusionList), `scripts/public-api/ui_drive_test.mjs` (settings card create → show-once → list → revoke). Route + model matrices updated (public v1 + api-keys routes, `ApiKey` model) so the isolation suite stays green by construction. Test gate: tsc 0, db:parity empty migration, build 0, test:isolation 124/0, functional + verify_step + UI-drive green. |
| v1.16.4 | 2026-09-04 | **Transcript → Evidence Chain — human-confirmed extraction (SAMS-013, Phase 4 Feature A — the signature differentiator).** AI proposes evidence + suggested actions per checklist item; the assessor confirms/edits/rejects EACH before anything links. **Nothing links while status=Proposed.** **(1) Additive schema:** `ExtractionProposal` record + `ProposalStatus` enum (`Proposed/Confirmed/Rejected`) + indexes; the proposal IS the attestation trail (status, source transcript + char-span reference `spanStart/spanEnd`, target checklist item, `proposedBy="AI"` + proposing user, `confirmedBy/rejectedBy` user + timestamp). Additive idempotent migration `scripts/db/migrations/20260904_add_extraction_proposal.ts` (**no `prisma db push`**; guard IF NOT EXISTS / duplicate_object / ON CONFLICT; also registers `ActivityLogType` rows `EVIDENCE_PROPOSAL_CREATED/CONFIRMED/REJECTED`; verified ×2). **(2) Scope-by-construction:** the extract + review routes take an access-checked `companyId`; the transcript AND the assessment must be the same company, and only that company's checklist items are ever fed to the extractor — cross-tenant targeting is impossible. **(3) Extraction (on-demand, never on upload):** `src/lib/evidence-extraction.ts` + `POST /api/admin/knowledgebase/transcript/[id]/extract-evidence` (`{companyId, assessmentId}`). The DeepSeek pipeline returns a strict-JSON item list; the service computes REAL char-span references by locating the excerpt in the transcript, then writes one PROPOSED `ExtractionProposal` per claim. A deterministic keyword extractor (`EVIDENCE_EXTRACTOR=keyword`) is used by the E2E harness (no LLM cost/flake); `deepseek` is the production default. **(4) Review:** `EvidenceExtractionPanel` rendered per expanded transcript in `TranscriptView` — "Extract evidence" (assessment picker) → proposals grouped by checklist item → per-item confirm (optional excerpt edit) / reject → bulk confirm-all-reviewed; rejected proposals are recorded and never resurfaced. **Confirmed → land on EXISTING rails:** evidence → `Attachment` (`destTable='AuditChecklistItem'`, `recId=<checklist item>`) on the checklist-item audit page; suggested action → a draft `Action` (`apAgreed=false`, human activates) on that checklist item's Finding. **Rejected → no link.** **(5) Routes:** `GET /api/admin/extraction/proposals` (scoped review list, default Proposed-only; `status=all` to see decisions + who/when), `PATCH /api/admin/extraction/proposals/[id]` (`{verdict:'confirm'|'reject', evidenceExcerpt?}`), `POST /api/admin/extraction/proposals/bulk-confirm`. Isolation matrices updated (model `ExtractionProposal` + 4 routes). **Test gate:** tsc green, additive migration ×2, test:isolation green, owner test-plan FUNCTION (a)-(e) + UI-driven (f) — see `scripts/evidence/`. |
| v1.16.4 | 2026-09-04 | **Outbound Notifications — per-company webhook transport + sweep + weekly digest (Phase 3a Feature B, SAMS-009).** (1) **Additive schema** (`scripts/db/migrations/20260904_add_outbound_notifications.ts`, idempotent, **no `prisma db push`**): `Company.notificationWebhookUrl TEXT?` (write-only secret), `NotificationDelivery` audit table (`notificationId?` FK→Notification `SetNull`; `channel` enum webhook; `companyId` FK→Company `Cascade`; `status` enum sent/failed; `responseCode Int?`; `attemptedAt`; `payloadPreview` ≤200; indexes companyId/notificationId/status+attemptedAt), and `NotificationType += ActionOverdue` (the sweep event). (2) **Webhook transport** in `src/lib/notifications.ts`: `postCompanyWebhook` reads the EVENT's own company URL, POSTs Slack-compatible `{"text": …}`, writes a delivery row, and NEVER throws (SAMS-006 fault-containment carries over). Wired into the four fabric emitters (EvidenceRequested/Submitted/Reviewed, CommentShared) — once per event at the company channel, never per-recipient; cross-tenant safe by construction (only the event's company URL is read). (3) **Sweep + digest** in `src/lib/notifications-outbound.ts` + two CRON_SECRET bearer-gated routes `POST /api/cron/notify-sweep` (actions newly-overdue ≤24h → one company-channel summary card + in-app `ActionOverdue` to client Admins) and `POST /api/cron/weekly-digest` (per subscribed company: SOC coverage %, new findings, overdue actions, open evidence requests). Routes are 401 without a matching `Authorization: Bearer <CRON_SECRET>` and 503 (fail-closed) if `CRON_SECRET` is unset; exempted from the NextAuth proxy (`src/proxy.ts` matcher) so they are reachable server-to-server. Trigger is external (Hermes box cron pilot v1; Railway cron documented). (4) **Client-Admin settings**: `GET/POST /api/portal/notifications-settings` (set/clear write-only URL, MASKED `{ configured }` response, never the URL) + `POST .../test` (send a test card + report delivery); shared gate `src/lib/portal-admin-gate.ts` (role=Admin + company membership, resolves company param > cookie > home > first); `/portal/settings` page + card + Settings nav tab; webhook URL never displayed (password field, cleared after save). (5) **Secret hygiene**: `notificationWebhookUrl` added to export `EXCLUSION_COLUMNS`; never in ActivityLog, API responses, or digest posts. Isolation matrices updated (`NotificationDelivery` model; `portal/settings` + `portal/notifications-settings[/test]` routes; cron routes global-exempt). Prisma client regenerated + committed. Verified: `tsc`, `db:parity`, `build`, `test:isolation` 124/0, functional (a)–(f) 44/0 incl. real local webhook receiver (delivery=sent/failed, 401 without secret, cross-tenant, secret-scan, clear=no further posts), UI drive (g) 12/0. **`CRON_SECRET`** must be added to the Railway env at landing + the local `.env` (generated; documented — never committed). |
| v1.16.3 | 2026-09-04 | **SAMS-010 round-1 review fixes (Conan).** **(1) Header active-company is server-resolved — no cross-company theme leak.** `PortalHeader` previously derived the active company client-side as `?companyId=` else `companies[0]`, but every portal page resolves it server-side as param > cookie > home > first (SAMS-007). For a multi-company portal user (home A + `UserCompany` B) with a `selectedCompanyId=B` cookie and no param, `companies[0]`=A made A's logo/colour render on B's `/portal/settings`. Fix: `src/app/portal/layout.tsx` resolves the active company SERVER-side via `resolvePortalCompanyId` (cookie > home > first) and passes `activeCompanyId` into `PortalHeader`, which themes/selects ONLY that id (param-primary kept so the header never disagrees with the page it wraps). The header selector also writes the `selectedCompanyId` cookie, so a multi-company Admin's choice survives tab navigation (which drops the param). Driven coverage: multi-company user with cookie=B → header logo/name/accent == B on `/portal` and `/portal/settings`, A unchanged; param=A → header == A; selector switch → survives tab nav. **(2) `PATCH /api/portal/company/theme` no longer clears omitted fields.** `logoUrl`/`primaryColor` defaulted to `null` and the update always wrote both, so `PATCH {primaryColor}` silently wiped an existing `logoUrl`. Fix: the route builds the update `data` conditionally — only keys present in the body are written; an explicit `null`/`""` clears that key; `{clear:true}` clears both; an empty `{}` body is a no-op. Covered by a functional check that a partial update preserves the other field. Gates re-verified green (tsc, db:parity empty migration, build, test:isolation 124/0, functional 41/0, UI-driven 27/0, DB-verify 6/0). |
| v1.16.2 | 2026-09-04 | **White-label theming — Company logo + colour on the portal (SAMS-010, Phase 3b Feature A).** **(1) Additive schema:** `Company` += `logoUrl String?` (TEXT, must be https at the API layer) + `primaryColor String?` (TEXT, validated `^#[0-9a-fA-F]{6}$` at the write route — Prisma `String?` cannot express the pattern). Additive idempotent migration `scripts/db/migrations/20260904_add_company_theme.ts` (**no `prisma db push`**; `ADD COLUMN IF NOT EXISTS` ×2; verified ×2). Both fields are **included** in the client-data export (brand, not secret). **(2) Scope-by-construction write:** `PATCH /api/portal/company/theme` (`src/app/api/portal/company/theme/route.ts`) manages the session user's ACTIVE portal company, resolved exactly like portal pages via `resolvePortalCompanyId` — a client-supplied `companyId` is never trusted, so a company-A Admin cannot theme company B and no cross-tenant write is possible. **Client Admin only** (other roles → 403; unauthenticated → middleware redirect). Body `{ logoUrl?, primaryColor? }` (empty/null clears that field) or `{ clear: true }` (clears both → back to SAMS default). Negative paths: invalid hex → 422; non-https logo URL → 422; `javascript:`/`ftp:` → 422. **(3) Portal-only theming v1 (settled #2):** `PortalHeader` renders the company logo in place of the text mark (silent text fallback on load failure — resilient to a bad URL and to an image that fails *before* hydration, via the img's own `load`/`error` events + a deferred `complete && naturalWidth===0` check) and sets a `--brand` CSS variable that drives the active nav-tab accent; the operator app stays SAMS-branded. **(4) Settings card** (`src/app/portal/settings/page.tsx` + `PortalThemeSettings`, same card family as the SAMS-009 webhook settings): set logo URL + colour swatch, live preview, save/clear; client Admin only. **(5) No per-user themes, no custom CSS** (injection/safety). Test gate: `tsc`, `db:parity`, `build`, `test:isolation` green; `scripts/whitelabel/functional_test.mjs` (a/b/c) + `ui_drive_test.mjs` (d/e/f) + `verify_step.ts` green against a throwaway DB seeded with client Admin A/B + client Assessor A (migration run ×2). |
| v1.16.1 | 2026-09-04 | **Pilot Onboarding Wizard — write-boundary re-validation (SAMS-008 review round-1).** `provisionUsers` now re-validates at the commit boundary via the SAME `isProvisionBlocked` rule the dry-run uses: a direct API commit (no dry-run) carrying a bad role, an empty name/username, or a duplicate username is refused with 4xx (409 duplicates / 422 bad rows) and ZERO writes — so the server enforces what the preview blocks, instead of trusting the client. `validateUserRows` gains a `missingFields` array (name/username) shared by the dry-run and the commit; the users route, the `OnboardingWizard.tsx` report panel, and the step-2 commit buttons were updated to surface it. P2 in the same round: docs typo fix (Temp passwords), step-2 commit-button busy feedback + "can take a minute" note. No schema change. |
| v1.16.0 | 2026-09-04 | **Pilot Onboarding Wizard — `/operator/onboarding` (SAMS-008, Phase 3a Feature A).** Provider-gated, dry-runnable operator motion to bring a new client company live, reusing the existing adoption + provision paths rather than reimplementing them. **(1) No schema change** — adds no tables/columns; reuses `Company`, `User`, `UserCompany`, and the shared bootstrap. **(2) Shared bootstrap lib** (`src/lib/bootstrap.ts`): extracted the SAMS001→target copy into `runBootstrap` (the single adoption path) + added read-only `previewBootstrap` that recomputes the copyable counts by mirroring the copy's linking logic (SAMS001 has 65 requirements with a `null` processAreaId — a naive COUNT would over-count and diverge from the commit result). The copy is now BATCHED (`createMany`/`createManyAndReturn`) for the ~5,500-row remote DB instead of a per-row loop. Both the legacy `/api/admin/company/[id]/bootstrap` route and the wizard call `runBootstrap`. **(3) Onboarding provider-plane lib** (`src/lib/onboarding.ts`): `validateUserRows` (DRY-RUN — duplicates existing/batch, invalid roles (only Assessor/Admin provisionable), unresolved managers + manager-resolution rate), `provisionUsers` (COMMIT — TRANSACTIONAL per company: any mid-batch failure rolls back to ZERO partial users; creates users + `UserCompany` mappings + bcrypt temp passwords held ONLY in a module-level transient store keyed by `wizardId`), `consumeTempPasswords` (one-time reveal — deletes the entry), `buildOnboardingReport` (full validation report + `approvedForGoLive`). **(4) APIs** (all `requireProvider`): `POST /api/operator/onboarding/company` (step 1, duplicate-companyID 409), `GET/POST /api/operator/onboarding/content` (step 2, orchestrate bootstrap — dry-run counts / commit), `POST /api/operator/onboarding/users` (step 3, dry-run report / transactional commit), `POST /api/operator/onboarding/finalize` (step 4, report + one-time password reveal). **(5) UI**: `OnboardingWizard.tsx` 4-step ... [truncated]
| v1.15.2 | 2026-09-04 | **Portal multi-company default-company resolution (SAMS-007).** `resolvePortalCompanyId` (`src/lib/portal.ts`) previously returned `null` for a user with 2+ `UserCompany` links whenever no `?companyId=` param was present (the cookie was never read), so a legitimate multi-company client landed on the misleading "You aren't linked to a company" empty state. **New default order when no valid `?companyId=` is supplied:** (1) `selectedCompanyId` **cookie** (server-read via `cookies()` in `src/lib/portal-server.ts`, matching the main app's URL-param-primary/cookie-fallback convention) → (2) `User.companyId` (**home** company) → (3) **first company by `companyID`** (ascending, deterministic last resort). Ratified precedence: **param > cookie > home > first-company-by-companyID**. Empty state is now shown **ONLY** for genuinely zero-company users. A cross-tenant `?companyId=` that is not in the user's mapped set is ignored and falls through to the default — it never leaks. No schema change (touched only `src/lib/portal.ts`, `src/lib/portal-server.ts`, `scripts/isolation/run.ts`, `scripts/isolation/sams007.ts`); no prod migration required. Isolation suite gains a driven SAMS-007 section (scenarios a–e: 3-company home=SMDS→SMDS not empty state; cookie→OGP; param beats cookie beats home; zero-company→empty state; single-company regression + cross-tenant param falls through/never leaks). |
| v1.14.0 | 2026-09-04 | **Client Portal — the primary client interface (SAMS-005, Phase 2b, Feature A).** **(1) Additive schema:** `Finding` += `managementResponse` (TEXT, ≤2000), `managementResponseAt` (TZ), `managementResponseById` (FK→`User`, `onDelete SetNull`) + `@@index([managementResponseById])` + `User.managementResponses` back-relation; additive idempotent migration `scripts/db/migrations/20260904_add_finding_management_response.ts` (**no `prisma db push`**; also registers `ActivityLogType.MANAGEMENT_RESPONSE_SAVE`). Run twice against the throwaway DB; db:parity zero drift on `Finding`. **(2) Scope-by-construction helpers** (`src/lib/portal.ts` + `src/lib/portal-server.ts`): every read pins `companyId` resolved from the session user's OWN company (or one of their `UserCompany` mappings); content rule = only `SharedWithClient` + client-authored comments (provider-`Internal` never enters a portal query — reuses the fabric `clientVisibleWhere()`); findings/actions traverse `Assessment.companyId`. Reads: `getPortalDashboard` (SOC coverage per PA + overall = #51 semantics `% FullyComply of assessed`), `getPortalFindings`, `getPortalActions` (overdue flag), `getPortalRequests` (current user, deep-link to fabric), `getPortalActivity` (composed feed, newest first, 50/page, from evidence-request events + shared comments + findings + SOC audits — never raw `ActivityLog`). **(3) Portal routes** (server components): `/portal` (dashboard), `/portal/findings`, `/portal/actions`, `/portal/requests`, `/portal/activity` (paginated) + the ONLY write `POST /api/portal/findings/[id]/management-response` (client Assessor+ of the owning company → stamps text/At/By; Interviewee → 403; >2000 → 422; empty clears; cross-tenant → 403; writes `MANAGEMENT_RESPONSE_SAVE` ActivityLog). **(4) Chrome + landing:** `src/proxy.ts` set `x-portal-route` for `/portal*` so the root layout suppresses `NavBar`/`MobileNav`; client `PortalHeader` renders simplified chrome (company name + a selector limited to the user's OWN mappings, card-grid nav, "App" link for Assessor+, sign-out). Root `/` rule (settled #6): no-`providerRole` users → `/portal`; providerRole users → existing targets. **(5) Isolation:** route-matrix drift detector extended to also scan portal PAGE routes; added portal routes + the two pre-existing fabric orphans (`comments`, `evidence-requests`, `evidence-requests/[id]`) to `route_matrix.json`; added `Comment`/`EvidenceRequest` to `model_matrix.json`; new section 4 portal cross-tenant probe (company-A/B dashboard/findings/actions/activity — zero other-tenant identifiers, dashboard totals match own-company requirement counts). Test gate: `tsc`, `db:parity`, `build`, `test:isolation` (111 checks) green; `scripts/portal/` functional (35) + UI-drive (23) + DB-verify green against a throwaway DB seeded with client Assessor + Interviewee A, client B, no-company user. |
| v1.13.18 | 2026-09-04 | **Conversation Fabric — threaded comments + evidence-request pipeline (SAMS-004, Phase 2a).** (1) **Additive schema:** one polymorphic `Comment` model (`entityType`+`entityId`, mirroring the AttachmentMapping destTable/recId convention; entities v1 `Finding`/`EvidenceRequest`; flat threads via `parentCommentId`, append-only v1; `authorPlane` Provider/Client derived server-side, per-comment `visibility` Internal/SharedWithClient) + `EvidenceRequest` DRL unit (state machine `Draft→Requested→Submitted→Accepted\|Rejected(→resubmit)\|NotApplicable`, files via the EXISTING polymorphic Attachment system `destTable='EvidenceRequest'`; optional FK to Assessment, requestedBy/requestedFrom FK to User). New enums `CommentAuthorPlane`, `CommentVisibility`, `EvidenceRequestStatus`. Additive idempotent migration `scripts/db/migrations/20260904_add_conversation_fabric.ts` (**no `prisma db push`**); also registers `ActivityLogType` rows `EVIDENCE_REQUEST_CREATED`/`EVIDENCE_REQUEST_STATUS`. (2) **APIs:** `GET/POST /api/comments` (visibility-filtered thread list — client sessions never see provider-`Internal`; POST derives `authorPlane` from session, rejects client `visibility=Internal` → 400, cross-company target → 403); `POST /api/evidence-requests` (Draft, `EVIDENCE_REQUEST_CREATED` audit), `GET /api/evidence-requests` (role-scoped, `?mine=1` for the requestee), `PATCH /api/evidence-requests/[id]` (state machine — 409 on invalid transitions, 422 on submit w/o note OR attachment, `EVIDENCE_REQUEST_STATUS` row with before/after). Requestee can only `submit` on their own request; assessor/provider drive `send`/`accept`/`reject`/`na`. (3) **UI:** reusable `CommentThread` (🛡 provider badge, 🔒/🌐 visibility badges, composer visibility toggle for provider authors only) on Finding cards + EvidenceRequest detail; 📨 Evidence tab on `/fla/[id]` (assessor: list/create/accept/reject, `UserSearchSelect` requestee picker); `/fla/my-evidence-requests` (requestee: cards, submit note+file, resubmit sees reviewNote, overdue red). New authz `requireAssessorOrProvider`; `useSession` + next-auth types expose `providerRole`. **Review hardening (round 1, 2026-09-04):** `POST /api/evidence-requests` now validates the requestee belongs to the target company (`User.companyId` OR a `UserCompany` mapping) → 400 otherwise, enforced for every plane (provider included — a mistyped username fails loud, never leaks another company's titles/instructions into a requestee's `?mine=1` inbox); cross-company requestee covered by a functional-test negative. State-machine note: v1 accepts `accept` directly from `Requested` and `accept`/`na` from `Rejected` (looser than the linear `Draft→Requested→Submitted→Accepted|Rejected(→resubmit)` DRL) — assessor-convenient, intentional for v1. On resubmit (`Rejected→Submitted`) the stale `reviewNote` is cleared and the requestee UI shows the review note only on a `Rejected` card. |
