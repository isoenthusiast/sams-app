...
Provider-gated with `session.user.providerRole` (non-provider → 403). Every write step is `dryRun`-first: the wizard validates/previews and the client only commits when the report is clean. Temperatures never appear in any API response except `/finalize`.
