...
| v1.2.0 | 2026-07-22 | Multi-assessor support (AssessmentAssessor junction), Interviewee role with My Interviews dashboard, activity template auto-creation (6 default activities per assessment), Document Review dedicated UI section, interviewee acceptance flow scaffolding, Assessment PUT endDate duplicate fix. Activities API rewritten to raw SQL to bypass PrismaPg adapter schema caching. Backfill endpoint for existing assessments. |
| v1.3.0 | 2026-07-22 | Admin User Management: UserManager component with full CRUD (add/edit/delete users), role assignment including Interviewee, company multi-select, password reset, self-delete prevention. API: POST/PUT/DELETE /api/admin/users. |
| v1.4.0 | 2026-07-22 | Phase F: Attachment Integration — added AttachmentList to Sample Selection and Finding & Actions tabs. Polymorphic AttachmentMapping supports destTable=Sample/Finding. |
| v1.5.0 | 2026-07-22 | Phase I & G specs settled via grill-with-docs. Control health deduction formula (quarterly reset, cumulative per severity, outstanding-action-gated). Action closure fields (closureDate, closureEvidence + AttachmentList). CONTEXT.md glossary + ADR-0001 created. Grill-with-docs skills adopted from Matt Pocock. |
