...
export function GamificationWidget({ userId }: Props) {
  const [data, setData] = useState<{ overallXP: number; latestTrack: string | null; latestTrackXP: number; tracks: Array<{ name: string; xp: number; level: string }> } | null>(null);
    ...
    <div className="flex items-center gap-3 px-3 py-2 bg-gradient-to-r from-slate-800 to-slate-700 rounded-lg text-white text-sm">
      ...
      {data.latestTrack && (
        <div className="flex-1 min-w-0 border-l border-slate-600 pl-3">
          ...
          <div className="font-medium truncate">{data.latestTrack}
            <span className="text-emerald-400 text-xs ml-1">+{data.latestTrackXP || 0}xp</span>
          </div>
        </div>
      )}
