...
export async function POST(request: Request) {
  ...
  const { name, statement, controlType, processAreaId, companyId, knowledge } = body;
  if (!name?.trim()) return NextResponse.json({ error: "name is required" }, { status: 400 });
  const ctrl = await prisma.control.create({
    data: { name: name.trim(), statement: statement?.trim() || "", controlType: controlType || "Administrative", processAreaId: processAreaId || null, companyId: companyId || null, knowledge: knowledge?.trim() || null },
